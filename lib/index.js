...
function check({
  ...
  task(function* () {
    let cleanup = [];
    try {
      let tmp_dir = yield _ => tmp.dir({
	unsafeCleanup:true
      }, (err, path, dispose) => {
	cleanup.push(dispose);
	_(err, path);
      });
      let version_array = yield getVersionsFromDist(distUrl, currentVersion, approveVersion); 
      if (version_array.length === 0) {
        return null;
      }

      let best_version = yield getBestVersion(version_array);
      ...
      // download signatures in cleartext
      let cleartext_sig_url = `${version_url}/SHASUMS256.txt`;
      let cleartext_sig_body = yield downloadAsString(cleartext_sig_url);
      ...
      hyperquest(encrypted_sig_url).pipe(child.stdin);
...
function* getVersionsFromDist(distUrl, currentVersion, approveVersion) {
